...
void ABullseyeEmitter::Emit()
{
	...
	for(const auto& Velocity : EmitVelocityArray)
	{
		...
		if (auto FoundComponent = Bullseye->GetComponentByClass(UShootOutMovementComponent::StaticClass()))
		{
			if (UShootOutMovementComponent* ShootOutMovementComponent = Cast<UShootOutMovementComponent>(FoundComponent))
			{
